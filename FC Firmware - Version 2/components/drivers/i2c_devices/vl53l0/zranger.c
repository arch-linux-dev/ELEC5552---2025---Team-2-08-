...
// Pin definitions for shutdown pins on ToF sensors
#define XSHUT1_PIN  35 // Bottom
#define XSHUT2_PIN  27 // Front
#define FWD_TOF_ADDRESS 0x30 // new address for front sensor


void zRangerInitBothSensors(void)
{
  if (isInit)
    return;

  // Configure XSHUT pins as outputs
  gpio_set_direction(XSHUT1_PIN, GPIO_MODE_OUTPUT);
  gpio_set_direction(XSHUT2_PIN, GPIO_MODE_OUTPUT);

  // Bring front sensor out of reset
  gpio_set_level(XSHUT1_PIN, 0);
  gpio_set_level(XSHUT2_PIN, 1);
  DEBUG_PRINTI( "Shutting down bottom ToF");

  // Small delay to allow sensors to boot up
  vTaskDelay(pdMS_TO_TICKS(10));

  // Initialize sensor and change its address
    DEBUG_PRINTI( "Initialising I2C bus for front ToF");
    frontdev.I2Cx = I2C1_DEV;                   // I2C bus
    frontdev.devAddr = VL53L0X_DEFAULT_ADDRESS;
    i2cdevInit(frontdev.I2Cx);

          DEBUG_PRINTI( "Setting new address");
        vl53l0xSetI2CAddress(&frontdev, FWD_TOF_ADDRESS);

  // Bring both sensor out of reset
      DEBUG_PRINTI( "Turning on both sensors");

  gpio_set_level(XSHUT1_PIN, 1);
  gpio_set_level(XSHUT2_PIN, 1);

   // Small delay to allow sensors to boot up
  vTaskDelay(pdMS_TO_TICKS(10));

  // Initialise bottom sensor
      DEBUG_PRINTI( "Init bottom sensor");
...
void print_zrange_task(void *pvParameters) {
    while (1) {
        float z = zRangerGetMeasurementAndRestart(&dev); // function that returns distance in meters
        if (z < 1){
          DEBUG_PRINTI("Obstacle detected\n");
          //adjTiltSetpoint(0);
        }
        DEBUG_PRINTI("Range sensor: %.2f m\n", z);
        vTaskDelay(pdMS_TO_TICKS(1000)); // wait 1 second
    }
}
void zRangerInitBottom(void) // bottom sensor only
{
  if (isInit)
    return;
  ...
  // Bring bottom sensor out of reset
  gpio_set_level(XSHUT1_PIN, 1);
  gpio_set_level(XSHUT2_PIN, 0);
  DEBUG_PRINTI( "Shutting down front ToF");
...
void zRangerInit(void) // original func
{
  if (isInit)
    return;
  ...
  dev.timeout_start_ms = 0;
  dev.stop_variable = 0;
  dev.measurement_timing_budget_us = 32000;
  dev.measurement_timing_budget_ms = dev.measurement_timing_budget_us / 1000;
  ...
  vl53l0xInit(&dev, I2C1_DEV, true);
  // Print range every 1 second
  xTaskCreate(print_zrange_task, "print_zrange", 2048, NULL, 1, NULL);
  xTaskCreate(zRangerTask, ZRANGER_TASK_NAME, ZRANGER_TASK_STACKSIZE, NULL, ZRANGER_TASK_PRI, NULL);
...
void zRangerTask(void* arg)
{
  ...
  while (1) {
    TickType_t delay = M2T(dev.measurement_timing_budget_ms);
    if (delay < 1) delay = 1;
